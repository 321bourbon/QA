...
from utils.utils import (
    deduplicate_and_pad, extract_result, get_normality_definition, parse_questions,
...
# Supported values: gpt (default), gemini, internvl
...
CONFIG = {
    ...
    "filter_shot": 20,             # 论文 = 50；数量越大过滤越稳定，成本越高
...
def generate_main_questions(vlm, class_name, fewshot_imgs):
    ...
    for idx, img_path in enumerate(fewshot_imgs, 1):
        ...
        if len(response.strip()) >= 20:
            ...
            preview = response.strip()[:160].replace("\n", " ")
            ellipsis = "..." if len(response.strip()) > 160 else ""
            print(f"    描述预览: {preview}{ellipsis}")
        else:
            print(f"    [警告] 描述过短 ({len(response.strip())} 字符)，丢弃")
    ...
    if len(summary.strip()) < 20:
        ...
        return []
    preview = summary.strip()[:200].replace("\n", " ")
    print(f"  摘要预览: {preview}{'...' if len(summary.strip()) > 200 else ''}")
...
def main():
    setup_runtime_logger(log_dir="./runtime_logs", file_prefix=_LOG_PREFIX, redirect_stdout=True)
    random.seed(CONFIG["seed"])
    try:
        print("=" * 80)
        print("  LogicQA 训练流程 (Training Pipeline)")
        print("=" * 80)
        print(f"  配置 class_list          (类别列表)       : {CONFIG['class_list']}")
        print(f"  配置 normal_shot         (描述用样本数)   : {CONFIG['normal_shot']}")
        print(f"  配置 filter_shot         (过滤用样本数)   : {CONFIG['filter_shot']}")
        print(f"  配置 mainq_acc_threshold (主问题保留阈值) : {CONFIG['mainq_acc_threshold']}")
        print(f"  配置 seed                (随机种子)       : {CONFIG['seed']}")
        print("=" * 80)
        ...
        for cls_idx, class_name in enumerate(class_list, 1):
            ...
            if final_qs:
                save_main_questions(class_name, final_qs, vlm_name=_VLM_BACKEND)
                print(f"  最终保留主问题数 (final_main_questions): {len(final_qs)}")

                # ── 阶段5: 生成并缓存子问题 ──
                # 论文 Appendix A：单次调用，批量返回 5 个语义等价变体
                ...
                save_sub_questions(class_name, sub_q_dict, vlm_name=_VLM_BACKEND)
            else:
