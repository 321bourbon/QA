...
def main():
    print("=" * 80)
    print("LogicQA end-to-end runner (Gemini)")
    ...
    for run_idx in range(1, NUM_RUNS + 1):
        ...
        results_dir = f"./test_results/run_gemini_{run_idx}"
        env = {
        ...
        ok_test, t_test = run_command([sys.executable, "test_gemini.py"], env)
        print(f"test_gemini.py: {'OK' if ok_test else 'FAIL'} ({t_test:.1f}s)")
        if not ok_test:
            continue
